...
When you add `--dry-run` to any `tbdflow` command, the tool won’t execute anything. Instead, it prints the Git commands it *would* have run, in the order they’d normally run.

- **Transparency**: No hidden steps, no magic. You see exactly what `tbdflow` is doing for a `commit`, `complete`, or `sync` command. That visibility hopefully builds trust and confidence.
